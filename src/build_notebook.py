...
cells.append(md_cell(
    ...
    "> **Note:** Requires `data/alibaba_features.csv` with a `failed` column aligned to the Borg feature schema. "
    "If the file is absent the cell documents the omission and saves a placeholder CSV.\n\n"
    "Document the omission in **Section 5** of the IEEE paper if the dataset is unavailable."
))
cells.append(code_cell(
    "import os\n"
    "\n"
    ...
    "    assert 'failed' in df_ali.columns, \"Alibaba dataset must have 'failed' column\"\n"
    "\n"
    ...
    "else:\n"
    "    print('Alibaba dataset not found. Cross-dataset validation skipped.')\n"
    ...
    "                ).to_csv(f'{RES_DIR}/cross_dataset_alibaba.csv', index=False)\n"
))
...
cells.append(md_cell(
    ...
    "| `cross_dataset_alibaba.csv` | `outputs/results/` | Omission documented |\n"
